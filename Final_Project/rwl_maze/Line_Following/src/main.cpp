...
float getPosition(uint8_t lineArray[13]) {
    int count = 0;
    float sum = 0;
    for (int i = 0; i < 13; i++) {
        if (lineArray[i] == 1) {
...
void runWiFiExchange() {
  ...
  for (int loopCount = 0; loopCount < 10; loopCount++) {
    if (client.connected()) {
      ...
      while (client.available()) {
        ...
        finalText[sizeof(finalText) - 1] = '\0';
      }
  ...
  // After 500 loops, we are done with WiFi. finalText has the last response.
  Serial.println("Finished 500 WiFi loops.");
  Serial.print("Final text stored: ");
...
void loop() {
  ...
  runWiFiExchange(); //Get verbal command from server
